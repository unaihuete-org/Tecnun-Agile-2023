...
1. Sign in using the Microsoft account associated with your Azure DevOps subscription (your university account).

    ![](https://azuredevopslabs.com/labs/azuredevops/prereq/images/000.png)

1. **Accept** the permission requests for accessing your subscription.

1. Select the **TecnunAzure2023** Azure DevOps organization and enter the project name **"Parts Unlimited NAME"**. Click **Choose Template**.

    ![](https://azuredevopslabs.com/labs/azuredevops/prereq/images/001.png)

1. Select the **PartsUnlimited** template and click **Select Template**.

    ![](https://azuredevopslabs.com/labs/azuredevops/prereq/images/002.png)

1. Click **Create Project** and wait for the process to complete.

    ![](https://azuredevopslabs.com/labs/azuredevops/prereq/images/003.png)


1. Click on **Navigate to project**.

1. Open the settings page using the **Project settings** navigation located at the bottom left of the page.

     ![](images/settings.png)

1. Go to **Project setttings** and change visibility to **Public**.
...
1. Navigate to **Board \| Dashboards**.

    ![](images/011.png)

1. Select the **Overview** dashboard for **PUL-Web Team**.


1. There are many ways to create work items in Azure DevOps, and we'll explore a few of them. Sometimes it's as simple as firing one off from a dashboard. **Add a widget**  and add a **New work Item** widget. Click on **Done editing**

1. In the **New Work Item** form, type **"Product training"** and select the **Epic** type. Click **Create**.
...
1. Note that the capacity view has been updated to reflect your available bandwidth. This exact number may vary, but for the screenshots here, that sprint capacity is 11 hours (1 hour per day over 11 working days). (It may be 10 depending on the time the lab is done).
...
1. Add a rule for **Priority = 1**. This will set all cards with priority **1** to  red. You can add more clauses in the query if required. 
    
    ![](images/styles_kanban2.png)

1. Click **Save and close** to save the styling rule.

1. Open the work item **As a customer, I want to view new tutorials** and change priority to **1**, **Save and Close**. The backlogs with priority **1** is now red and very easy to identify.
...
1. Set the first term to **Work Item Type = Task** and the second term to **Area Path = Parts Unlimited NAME\PUL-Web**.
...
1. You can now add this chart to a dashboard. Add it to the **Product training one.
...
1. Set the name of the inherited process to **Customized Scrum NAME** and click **Create process**.

    ![](images/110.png)

1. Open **Customized Scrum NAME**. You may need to refresh the browser for this to become visible.
...
1. From the context menu for **Parts Unlimited NAME**. Select **Change process**.

    ![](images/119.png)

1. Select the **Customized Scrum NAME** process and click **Save**.
...
1. Open the **Parts Unlimited NAME** project.
